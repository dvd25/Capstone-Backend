...
exports.create = async (req, res) => {
    ...
    sql.query('SELECT * FROM users WHERE email = ?', [req.body.email], async function (error, results, fields) {
        // If there is an issue with the query, output the error
        if (error) throw error;
        // If the account exists
        if (results.length > 0) { //if there is a user found
            res.status(409).send({ rejected: true, message: 'Account already exists with the email.' })
            res.end();
        }
        else {
            try {
                ...
                User.create(user, (err, data) => {
                    ...
                    else res.status(200).send({data, rejected: false});
                });
        ...
        if (email && password) {
            ...
            sql.query('SELECT * FROM users WHERE email = ?', [email], async function (error, results, fields) {
                ...
                if (results.length > 0) {
                    ...
                    res.status(404).send({ message: 'Email not found', existingEmail: 'false' });
                }
                res.end();
            });
        } else {
